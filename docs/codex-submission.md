...
Recorded here because two files link it as the primary install path — `README.md` and
`site/index.html` — so a re-listing that mints a new id has callers to update, not just a
bookmark. Deliberately not a table row: `check_starter_prompts` reads every row whose first

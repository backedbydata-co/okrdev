...
|--------|----------|--------|
| docs/skills cross-file coherence | any reviewer-class contradiction shipped to main and left >1 week | manual / a coherence pass on the diff |
| Install footprint | install writes anything beyond `okrdev/` + the marked CLAUDE.md block (+ opt-in `.github/` at L2) | `git status` after a fresh install |
